...
): { result: { newLevel: number; hpGained: number; proficiencyBonus: number; asiAvailable: boolean; log: string }; stateOps: StateOperation[] } {
  const stateOps: StateOperation[] = [];

  if (!canLevelUp(character)) {
    return {
      result: { newLevel: character.level, hpGained: 0, proficiencyBonus: 0, asiAvailable: false, log: '经验值不足，无法升级' },
      stateOps,
    };
  }
