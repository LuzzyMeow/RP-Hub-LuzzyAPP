...
- **请求 1 — 工具决策**：AI 查看上下文后自主决定调用哪些工具、传入什么参数。系统提示仅含工具决策指令 + 可用工具列表，不含角色扮演设定，避免模型进入角色扮演模式
- **请求 2 — CoT 思考链**：基于工具结果输出完整思考过程。系统提示含角色预设 + 世界书 + CoT 协议
- **请求 3 — 正文**：基于 CoT 输出正文。系统提示含角色预设 + 世界书（无 CoT 协议，避免与末尾 user 指令冲突）
...
`chat-slice.ts` thinking 步骤按 phase（1/2/3）独立创建：phase=1 "工具决策分析"、phase=2 "深度推理"、phase=3 "组织回复"，不再合并覆盖到同一个"模型思考"节点。

### 🔧 系统提示 phase 感知

`chatService.ts` `buildContext` 新增 `phase` 参数（1/2/3）：请求 1 仅注入工具决策协议 + 可用工具列表，跳过角色预设/世界书/文风/角色定义；请求 2 完整系统提示含 CoT 协议；请求 3 完整系统提示不含 CoT 协议（避免与末尾 user 指令冲突）。

### ⚡ 流式输出恢复

`chat-slice.ts` `parseCot` 流式过程中恢复 `includeUnclosed=true`，cot 内容随 SSE chunk 增量增长。

### 🧹 工具页全局模式移除

`tools.tsx` 删除"工具全局模式"选择卡片，默认所有已启用工具在请求 1 对 AI 可见。

### 🏠 LUZZY 预设内置

`preset.tsx` 预设列表过滤隐藏 LUZZY 预设，纯代码内置。
